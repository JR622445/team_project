...
while True : 
	...
	myList = ['Kebab Halal Pizza', 'Star Biryani', 'Al-Karam Sweets', 'Apna Farm', "Shabi's", 'Tun Travel', 'WildFish', 'Autorebex', 'Wealth Managment', 'Immigration', 'NageenabyNaureen', 'Mann Lawyers', 'Gazelle Media', 'Graphic Design', 'Quran Program', 'Children Clothing', 'Halal Commerce', 'Wave Fitness', 'Think Travel', 'EleganceBeard', 'Dignitii']

	#Joshua, Kianoush: lines 33-65
	newInput = str(input())
	...
	if newInput == '4':
		print('Add the business name to the list.')
		...
		print("\n".join(Category))
		newInputSeven = input()
		while newInputSeven not in Category:
			print("That Category does not exist try again:")
			newInputSeven= input()
			
		print(newInputFive + ' - ' + newInputSix)
		myList.append(newInputFive)
		myList2.append(newInputFive)
...
# Joshua 67-182
	...
	#Kianoush: lines 184-299
		...
		if typeInput == 'Food': 
			...
			otherInput = input()
			print()
			while otherInput != 'Wealth Managment ' and otherInput != 'Mann Lawyers' and otherInput !=' Quran Program' and otherInput != 'Wave Fitness' and otherInput != 'Think Travel':
				print("Invalid business try again:")
				otherInput= input() 
			if otherInput == 'Wealth Managment':
				print(WealthManagment)
			elif otherInput =='Mann Lawyers':
				print(MannLawyers)
